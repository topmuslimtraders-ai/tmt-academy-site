    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <div class="glass-card glass-card-hover rounded-2xl overflow-hidden border border-cardBorder flex flex-col">
        ...
        <div class="p-5 flex flex-col flex-grow">
          ...
          <p class="text-xs text-slate-400 mt-2 flex-grow whitespace-pre-line line-clamp-4">${esc(a.text)}</p>
          <div class="flex items-center gap-2 mt-4 pt-4 border-t border-cardBorder/60">
            <button type="button" onclick="openArticleModal(${i})" class="flex-grow inline-flex items-center justify-center gap-1.5 text-xs font-bold bg-emeraldGreen/15 text-emeraldGreen px-3 py-2.5 rounded-lg hover:bg-emeraldGreen/25 transition">
              <i class="fa-solid fa-book-open"></i> To'liq o'qish
            </button>
            <button type="button" onclick="openArticleModal(${i})" title="Ulashish" class="w-10 h-10 flex-shrink-0 inline-flex items-center justify-center bg-cardBorder/40 text-slate-300 rounded-lg hover:bg-accentBlue/20 hover:text-accentBlue transition">
      ...
      <div class="glass-card glass-card-hover rounded-2xl overflow-hidden border border-cardBorder flex flex-col">
        ...
        <div class="p-6 flex flex-col flex-grow">
          ...
          <div class="flex items-center gap-2 mt-4">
            <button type="button" onclick="openPdfModal(${i})" class="flex-grow inline-flex items-center justify-center gap-1.5 text-xs font-bold bg-accentPurple/15 text-accentPurple px-3 py-2.5 rounded-lg hover:bg-accentPurple/25 transition">
              <i class="fa-solid fa-eye"></i> Onlayn o'qish
            </button>
            <a href="${esc(p.url)}" target="_blank" download title="Yuklab olish" class="w-10 h-10 flex-shrink-0 inline-flex items-center justify-center bg-cardBorder/40 text-slate-300 rounded-lg hover:bg-emeraldGreen/20 hover:text-emeraldGreen transition">
              <i class="fa-solid fa-download"></i>
            </a>
            <button type="button" onclick="openPdfModal(${i})" title="Ulashish" class="w-10 h-10 flex-shrink-0 inline-flex items-center justify-center bg-cardBorder/40 text-slate-300 rounded-lg hover:bg-accentBlue/20 hover:text-accentBlue transition">
              <i class="fa-solid fa-share-nodes"></i>
            </button>
          </div>` : `<span class="mt-4 inline-flex items-center justify-center text-xs font-bold bg-cardBorder/40 text-slate-500 px-4 py-2.5 rounded-lg">Tez orada</span>`}
        </div>
      </div>`).join('')}
    </div>
  </div>
...
<div id="readerOverlay" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-[60] hidden items-center justify-center p-4">
  <div class="glass-card border border-cardBorder rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden bg-cardBg">
    <div class="flex items-start justify-between gap-3 p-5 border-b border-cardBorder">
      <h3 id="readerTitle" class="text-base font-bold text-white pr-2"></h3>
      <button type="button" onclick="closeReaderModal()" aria-label="Yopish" class="text-slate-400 hover:text-white text-2xl leading-none flex-shrink-0">&times;</button>
    </div>
    <div id="readerBody" class="p-6 overflow-y-auto text-sm text-slate-300 leading-relaxed whitespace-pre-line"></div>
    <div class="flex items-center gap-2 p-4 border-t border-cardBorder">
      <button type="button" onclick="shareCurrent('whatsapp')" class="flex-grow inline-flex items-center justify-center gap-2 text-xs font-bold bg-[#25D366]/15 text-[#25D366] px-3 py-2.5 rounded-lg hover:bg-[#25D366]/25 transition">
        <i class="fa-brands fa-whatsapp text-sm"></i> WhatsApp
      </button>
      <button type="button" onclick="shareCurrent('telegram')" class="flex-grow inline-flex items-center justify-center gap-2 text-xs font-bold bg-accentBlue/15 text-accentBlue px-3 py-2.5 rounded-lg hover:bg-accentBlue/25 transition">
        <i class="fa-brands fa-telegram text-sm"></i> Telegram
      </button>
      <button type="button" onclick="shareCurrent('copy')" title="Havolani nusxalash" class="w-11 h-11 flex-shrink-0 inline-flex items-center justify-center bg-cardBorder/40 text-slate-300 rounded-lg hover:bg-goldAccent/20 hover:text-goldAccent transition">
...
<div id="pdfOverlay" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-[60] hidden items-center justify-center p-2 sm:p-4">
  <div class="glass-card border border-cardBorder rounded-2xl w-full max-w-4xl h-[92vh] flex flex-col overflow-hidden bg-cardBg">
    <div class="flex items-center justify-between gap-3 p-4 border-b border-cardBorder">
      <h3 id="pdfTitle" class="text-sm font-bold text-white pr-2 truncate"></h3>
      <div class="flex items-center gap-2 flex-shrink-0">
        <a id="pdfDownloadLink" href="#" target="_blank" download title="Yuklab olish" class="w-9 h-9 inline-flex items-center justify-center bg-emeraldGreen/15 text-emeraldGreen rounded-lg hover:bg-emeraldGreen/25 transition">
          <i class="fa-solid fa-download"></i>
        </a>
        <button type="button" onclick="closePdfModal()" aria-label="Yopish" class="text-slate-400 hover:text-white text-2xl leading-none px-1">&times;</button>
      </div>
    </div>
    <iframe id="pdfFrame" src="" class="flex-grow w-full bg-white" title="PDF ko'rish"></iframe>
    <div class="flex items-center gap-2 p-3 border-t border-cardBorder">
      <button type="button" onclick="shareCurrent('whatsapp')" class="flex-grow inline-flex items-center justify-center gap-2 text-xs font-bold bg-[#25D366]/15 text-[#25D366] px-3 py-2.5 rounded-lg hover:bg-[#25D366]/25 transition">
        <i class="fa-brands fa-whatsapp text-sm"></i> WhatsApp
      </button>
      <button type="button" onclick="shareCurrent('telegram')" class="flex-grow inline-flex items-center justify-center gap-2 text-xs font-bold bg-accentBlue/15 text-accentBlue px-3 py-2.5 rounded-lg hover:bg-accentBlue/25 transition">
...
// ---- Maqola o'qish + PDF onlayn ko'rish + Ulashish (WhatsApp/Telegram) ----
(function(){
  const articlesData = ${JSON.stringify((c.articles || []).map(a => ({ title: a.title || '', text: a.text || '' })))};
  const pdfsData = ${JSON.stringify((c.pdfs || []).map(p => ({ title: p.title || '', url: p.url || '' })))};
  const pageBaseUrl = (typeof window !== 'undefined' ? window.location.href.split('#')[0] : '');
  let currentShare = { title: '', url: '' };

  window.openArticleModal = function(i){
    const a = articlesData[i];
    if (!a) return;
    document.getElementById('readerTitle').textContent = a.title;
    document.getElementById('readerBody').textContent = a.text;
    currentShare = { title: a.title, url: pageBaseUrl + '#articles' };
  ...
  window.openPdfModal = function(i){
    const p = pdfsData[i];
    if (!p || !p.url) return;
    document.getElementById('pdfTitle').textContent = p.title;
    document.getElementById('pdfFrame').src = p.url;
    document.getElementById('pdfDownloadLink').href = p.url;
    currentShare = { title: p.title, url: p.url };
  ...
  window.shareCurrent = function(channel){
    if (!currentShare.url) return;
    const text = encodeURIComponent(currentShare.title + ' — Top Muslim Traders Academy');
  ...
  if (readerOverlay) readerOverlay.addEventListener('click', function(e){ if (e.target === readerOverlay) closeReaderModal(); });
  if (pdfOverlay) pdfOverlay.addEventListener('click', function(e){ if (e.target === pdfOverlay) closePdfModal(); });
  document.addEventListener('keydown', function(e){
    if (e.key === 'Escape') { closeReaderModal(); closePdfModal(); }
  });
})();
</script>
</body>
